...
void DebugCamera::Init(Vector3 rotation, Vector3 translation) {
	rotation_ = rotation;
...
void DebugCamera::Update() {
	...
	mousePos_ = Kujaku::Input::GetMousePos();

	if (Kujaku::Input::GetMouse(1)) {
	...
	if (Kujaku::Input::GetKey(DIK_W)) {
		translation_ += forward * kMoveSpeed;
	}
	if (Kujaku::Input::GetKey(DIK_S)) {
		translation_ -= forward * kMoveSpeed;
	}
	if (Kujaku::Input::GetKey(DIK_A)) {
		translation_ -= right * kMoveSpeed;
	}
	if (Kujaku::Input::GetKey(DIK_D)) {
		translation_ += right * kMoveSpeed;
	}
	if (Kujaku::Input::GetKey(DIK_Q)) {
		translation_.y += kMoveSpeed;
	}
	if (Kujaku::Input::GetKey(DIK_E)) {
		translation_.y -= kMoveSpeed;
	}
