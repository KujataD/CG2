...
Microsoft::WRL::ComPtr<IDxcBlob>
    CompileShader(const std::wstring& filePath, const wchar_t* profile, IDxcUtils* dxcUtils, IDxcCompiler3* dxcCompiler, IDxcIncludeHandler* includeHandler);
...
[[nodiscard]] // 戻り値を破棄してはいけない。
Microsoft::WRL::ComPtr<ID3D12Resource>
    UploadTextureData(ID3D12Resource* texture, const DirectX::ScratchImage& mipImages, ID3D12Device* device, ID3D12GraphicsCommandList* commandList);
...
#pragma region DXGIFactoryの生成(ウィンドウ生成後)
...
#pragma region コマンドキュー/コマンドアロケータ/コマンドリストを生成する
...
#pragma region レンダーターゲットビュー(RTV)を生成
...
#pragma region RasterrizerStateの設定を行う
...
#pragma region Vertex/PixelShaderをコンパイルする
...
#pragma region DirectionalLightのリソース
...
#pragma region Modelのリソース群の作成
	...
	// Model用のTransformationMatrix用のリソースを作る。Matrix4×4 1つ分のサイズを用意する
	Microsoft::WRL::ComPtr<ID3D12Resource> transformationMatrixResourceModel = CreateBufferResource(device.Get(), sizeof(TransformationMatrix));
	// データを書き込む
	...
	// 単位行列を書きこんでおく
	transformationMatrixDataModel->World = Matrix4x4::MakeIdentity();
	transformationMatrixDataModel->WVP = Matrix4x4::MakeIdentity();
...
#pragma region SpriteのTransformationMatrixResource/MaterialResource
...
#pragma region Shader Resource Viewを作る
...
#pragma region 解放処理
	...
	// ComPtrで管理しているものは自動で解放されるため、手動でのReleaseは行わない

	CloseWindow(hwnd);
#pragma endregion
	// リソースリークチェックはWinMain先頭のleakCheckのDestructorで行われる
	// COMの終了処理
	CoUninitialize();
	return 0;
}
...
DirectX::ScratchImage LoadTexture(const std::string& filePath) {
	...
	// WIC_FLAGS_FORCE_SRGB : sRGB空間で作られたモノとして読む。sRGBについては後々
